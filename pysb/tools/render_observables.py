...
def run(model):
    ...
    for oi,o in enumerate(obs_species_monomers):
        x = o.monomer_patterns[0]
        ## get the species IDs of observable monomers which are
        ## in the species list
        s_ids = [si for si,s in enumerate(model.species) if x.monomer in [sp.monomer for sp in s.monomer_patterns]]
...
#        s_ids = [si for si in s_ids if any([True for mp in model.species[si].monomer_patterns if x.monomer == mp.monomer and x.site_conditions == mp.site_conditions])]
        s_ids = [si for si in s_ids if any([True for mp in model.species[si].monomer_patterns if x.monomer == mp.monomer and all(item in mp.site_conditions.items() or item[1] == pysb.ANY for item in x.site_conditions.items())])]
