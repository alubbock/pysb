...
import pysb
import abc
from .core import SelfExporter
from .pattern import SpeciesPatternMatcher, ReactionPatternMatcher, \
    RulePatternMatcher
...
class TestSuite(object):
    ...
    >>> from pysb.modeltests import TestSuite, SpeciesExists, \
        SpeciesDoesNotExist
    ...
    Check that AMito() % mSmac() exists in the species graph (note this
    ...
    >>> ts.add(SpeciesExists(AMito() % mSmac()))

    This is the opposite check, that the complex above doesn't exist,
    which should of course fail:

    >>> ts.add(SpeciesDoesNotExist(AMito() % mSmac()))

    We can specify particular binding site combinations or states, too:

    >>> ts.add(SpeciesExists(AMito(b=1) % mCytoC(b=1)))
...
class SpeciesExists(SpeciesAssertion):
    ...
    """
    def check(self, model, **kwargs):
        self._last_result = kwargs['species_pattern_matcher'].match_species(
            self.pattern)
